...
function getPayload(status, description, job) {
    ...
    let payload = {
        embeds: [Object.assign({ title: statusOpts[status].status + (job ? `: ${job}` : ''), description: `Commit: [${sha_short}](${repoURL}/commit/${sha}) ${commitmsg}\nRef: ${ref}\n[Build](${workflowURL})`, color: statusOpts[status].color }, blame && { footer: { 'text': `Blame ${blame}!`, 'icon_url': blamea } })],
